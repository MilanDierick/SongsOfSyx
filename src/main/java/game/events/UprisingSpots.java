...
public final class UprisingSpots{
	...
	private ArrayList<UprisingSpot> spots = new ArrayList<>(RES.config().BATTLE.DIVISIONS_PER_ARMY);
	private int divI;
	...
	public int riot(int amount) { 
		...
		int sp = (int) Math.ceil((double)amount/(RES.config().BATTLE.MEN_PER_DIVISION));
	...
	private boolean nextDivision() {
		if (divI >= RES.config().BATTLE.DIVISIONS_PER_ARMY) {
			return false;
		}
