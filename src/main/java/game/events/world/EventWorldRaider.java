...
public class EventWorldRaider extends EventResource{
	...
	private static CharSequence ¤¤HasSpawned = "¤An army of rebels have been spotted. They are marching towards our capital.";
	...
	public final DOUBLE CHANCE = new DOUBLE() {
		...
		private double getChance() {
			...
			if (STATS.POP().POP.data().get(null) <= 85)
				return 0;
			
			if (FACTIONS.player().credits().credits() < 0)
				return 0;
			chance *= Math.pow(CLAMP.d((STATS.POP().POP.data().get(null)-85)/800.0, 0, 1), 0.5);
	...
	private WArmy spawn(int men, double training, double gear) {
		COORDINATE c = WORLD.PATH().rndDistOwn(FACTIONS.player().capitolRegion().cx(), FACTIONS.player().capitolRegion().cy(), 5);
		...
		protected RENDEROBJ makeSection() {
			...
			section.addRelBody(48, DIR.S, new GButt.ButtPanel(¤¤pay) {
				...
				protected void clickA() {
					if (!timer.hasExpired()) {
						FACTIONS.player().credits().inc(-timer.credits, CTYPE.MISC);
						SETT.ROOMS().STOCKPILE.removeFromEverywhere(0.1, RBIT.ALL, RTYPE.SPOILS);
