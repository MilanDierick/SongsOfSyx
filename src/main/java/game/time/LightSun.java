...
public final class LightSun {
	...
	public void apply(int x1, int x2, int y1, int y2, RGB tint) {
		...
		if (S.get().lightCycle.get() == 0) {
		...
		}else if (TIME.light().nightIs() && TIME.light().secondsLeft() < twilightSeconds) {
			...
			w.multiply(tint);
			set(w, 0, x1, x2, y1, y2);
		}else if (TIME.light().dayIs() && TIME.light().secondOf() < twilightSeconds/2) {
		...
		}else if(TIME.light().dayIs() && TIME.light().secondsLeft() < twilightSeconds/2) {
		...
		}else if(TIME.light().nightIs() && TIME.light().secondOf() < twilightSeconds) {
