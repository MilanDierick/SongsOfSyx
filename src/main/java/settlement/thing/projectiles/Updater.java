	...
	void update(final int i, float dd) {
		...
		while(dd > 0) {
			...
			if (d.z() <= 0) {
				...
				p.data.type(i).impact(p.data.level(i), x, y, d.dx(), d.dy(), d.dz());
				p.data.remove(i);
	...
	private boolean collide(int e, Data d, float destX, float destY, Projectile type) {
		...
		while (mag > 0) {
			...
			if (d.z()<= 0 ||  (d.z() > min && d.z() < max)) {
				...
				double mom = Math.sqrt(d.dx()*d.dx()+d.dy()*d.dy()+ d.dz()*d.dz())*p.data.type(e).mass(p.data.level(e));
				...
				type.impact(p.data.level(e), x*C.TILE_SIZE, y*C.TILE_SIZE, d.dx(), d.dy(), d.dz());
			...
			if (d.z() >= hel && d.z() < heh) {
				...
				for (ENTITY ent : SETT.ENTITIES().fill(rTile)) {
					if (intesects(ent, ox, oy, x, y)) {
						p.data.type(e).soundHit.rnd((int) (x * C.TILE_SIZE), (int) (y * C.TILE_SIZE), 1);
						double lxy =  d.dx()*d.dx() + d.dy()*d.dy();
						double l = lxy + d.dz()*d.dz();
						l = Math.sqrt(l);
						double mom = type.mass(p.data.level(e)) * l;
						double momExchange = ent.physics.getMass()*C.TILE_SIZE*7;
						
						DIR od = ent.speed.dir();
						vec.set(ox*C.TILE_SIZE, oy*C.TILE_SIZE, ent.body().cX(), ent.body().cY());
						double nY = ent.speed.y() + vec.nY()*mom*ent.physics.getMassI();
						double nX = ent.speed.x() + vec.nX()*mom*ent.physics.getMassI();
						ent.speed.setRaw(nX, nY);
						double dot = (1 -od.xN()*vec.nX() -od.yN()*vec.nY())*0.5;
						
						coll.pierceDamage = type.pierce(p.data.level(e));
						coll.dirDot = dot;
						coll.momentum = mom;
						coll.norX = -vec.nX();
						coll.norY = -vec.nY();
						...
						if (ent.isRemoved() && mom - momExchange > 0) {
							mom-= momExchange;
							mom/= type.mass(p.data.level(e));
							double ddd = mom/l;
							d.dxSet(d.dx()*ddd);
							d.dySet(d.dy()*ddd);
						}else {
							type.impact((p.data.level(e)), x*C.TILE_SIZE, y*C.TILE_SIZE, 0, 0, 0);
							p.data.remove(e);
							return true;
						}
