...
public class StatsNeedsDisease implements Updatable{
	...
	StatsNeedsDisease(Init init) {
		infected = new STAT.STATData("INFECTED", init, new INT_OE<Induvidual>() {
	...
	public void update16(Humanoid h, int updateR, boolean day, int updateI) {
		...
		if (st == INCUBATING) {
			if ((((i.randomness+TIME.seasons().bitsSinceStart()) >>7)&0b11) == 0 && LAW.curfew().is()) {
				status.set(i, NONE);
				count.set(i, 0);
