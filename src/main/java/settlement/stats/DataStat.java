...
import settlement.stats.Init.Pushable;
import snake2d.util.file.*;
import snake2d.util.misc.CLAMP;
import util.statistics.HISTORY_INT.HISTORY_INT_OBJECT;

abstract class DataStat implements HISTORY_INT_OBJECT<Race>, SAVABLE, Pushable{
	...
	void incrFull(Induvidual i, int d) {
		current[i.race().index] += d;
		current[stride-1] += d;
	}
	...
	static abstract class DataRaceI extends DataStat {
		...
		DataRaceI(Init init, INT_OE<Induvidual> data){
			super(init);
			this.data = data;
		}
