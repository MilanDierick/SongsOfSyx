...
public final class StatsAppearance extends StatCollection {
	...
	public final LIST<STAT> extraBits;
	...
	void changeHtype(Induvidual h, HTYPE old) {
		...
		if (old == null || (old != h.hType() && (old == HTYPE.CHILD || h.hType() == HTYPE.CHILD))){
			RTypeSpec s = h.race().appearance().child.spec;
			if (h.hType() != HTYPE.CHILD) {
				s = h.race().appearance().types.getC(gender.get(h)).spec;
			}
			for (int si = 0; si < extraBits.size(); si++) {
				extraBits.get(si).indu().set(h, s.extraOccurence[si] > RND.rFloat() ? 1 : 0);
			}
		}
	}
	
	StatsAppearance(Init init){
		...
		extraBits = new ArrayList<STAT>(
				new STAT.STATInduOnly("EXTRA1", init, init.count.new DataBit()),
				new STAT.STATInduOnly("EXTRA2", init, init.count.new DataBit()),
				new STAT.STATInduOnly("EXTRA3", init, init.count.new DataBit()),
				new STAT.STATInduOnly("EXTRA4", init, init.count.new DataBit())
				);
		
		dead = new STAT.STATInduOnly("DEAD", init, init.count.new DataBit());
		
		LinkedList<INT_OE<Induvidual>> tt = new LinkedList<>();
		tt.add(name);
		tt.add(customName);
		tt.add(gender);
		for (STAT s : extraBits)
			tt.add(s.indu());
