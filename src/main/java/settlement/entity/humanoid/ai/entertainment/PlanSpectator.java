...
final class PlanSpectator extends AIPLAN.PLANRES{
	...
	private static ROOM_SPECTATOR spec = null;
	
	public AiPlanActivation activate(Humanoid a, AIManager d, ROOM_SPECTATOR s) {
		spec = s;
	...
	protected AISubActivation init(Humanoid a, AIManager d) {
		if (!spec.service().accessRequest(a)) {
			spec.service().clearAccess(a);
			return null;
		}
		return walk.set(a, d);
	}
	...
	private ROOM_SPECTATOR s(AIManager d) {
		RoomBlueprint s = SETT.ROOMS().map.blueprint.get(d.planTile);
		if (s != null && s instanceof ROOM_SPECTATOR_HASER)
			return ((ROOM_SPECTATOR_HASER)s).spec();
		return null;
	}
	...
	private final R walk = new R() {
		...
		protected AISubActivation setAction(Humanoid a, AIManager d) {
			...
			COORDINATE c = spec.service().finder.reserve(a.tc(), spec.service().radius);
			if (c == null)
				return null;
			d.planTile.set(c);
	...
	private final R stand = new R() {
		...
		protected AISubActivation setAction(Humanoid a, AIManager d) {
			...
			if (d.planByte1 <= 0) {
				if (!AI.modules().entertainment.moduleCanContinue(a, d) || RND.oneIn(5)) {
					can(a, d);
					return null;
				}
