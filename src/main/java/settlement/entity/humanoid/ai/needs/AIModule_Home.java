...
public final class AIModule_Home extends AIModule{
		...
		if (STATS.HOME().GETTER.has(a)) {
			if ((AIModules.current(d) == this)){
				if (TIME.light().nightIs())
					return 3;
				return (int) ((a.indu().randomness()>>2) + TIME.days().bitsSinceStart() & 0b01) +1;
			}
			return (int) ((a.indu().randomness()>>2) + TIME.days().bitsSinceStart() & 0b01);
		}
	...
	private final AIPLAN sleep_home = new AIPLAN.PLANRES() {
		...
		private final Resumer fetch = new Resumer(¤¤name) {
			
			@Override
			protected AISubActivation setAction(Humanoid a, AIManager d) {
				if (shouldRes(a, d)) {
					AISubActivation ss = retry(a, d);
					if (ss != null)
						return ss;
			...
			private AISubActivation retry(Humanoid a, AIManager d) {
				...
				for (int ri = 0; ri < a.race().home().clas(a.indu().clas()).resources().size(); ri++) {
					if (STATS.HOME().shouldFetch(a, ri)) {
						m |= a.race().home().clas(a.indu().clas()).resources().get(ri).resource().bit;
					}
					
				}
				
				if (m != 0) {
					return AI.SUBS().walkTo.resource(a, d, m, 120);
				}
				return null;
			}
			...
			protected AISubActivation res(Humanoid a, AIManager d) {
				...
				STATS.HOME().fetchResource(a, d.resourceCarried());
