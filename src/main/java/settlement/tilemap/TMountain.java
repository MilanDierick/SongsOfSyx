...
public class TMountain extends Terrain.TerrainTile{
	...
	static TMountain make(Terrain t) throws IOException {
		...
		SPRITE icon = new SPRITE.Imp(ICON.BIG.SIZE) {
			...
			public void render(SPRITE_RENDERER r, int X1, int X2, int Y1, int Y2) {
				sheet.render(r, DIR.SE.mask(), X1, X1+C.T_PIXELS, Y1, Y1+C.T_PIXELS);
				sheet.render(r, DIR.SW.mask(), X1+C.T_PIXELS, X1+C.T_PIXELS*2, Y1, Y1+C.T_PIXELS);
				sheet.render(r, DIR.NE.mask(), X1, X1+C.T_PIXELS, Y1+C.T_PIXELS, Y1+C.T_PIXELS*2);
				sheet.render(r, DIR.NW.mask(), X1+C.T_PIXELS, X1+C.T_PIXELS*2, Y1+C.T_PIXELS, Y1+C.T_PIXELS*2);
			}
		};
		
		return new TMountain(t, sheet, mountain_ceiling, icon);
	}
	
	TMountain(Terrain t, TILE_SHEET sheet, TILE_SHEET sheetC, SPRITE icon) {
		super(t, new INFO(new Json(PATHS.TEXT().getFolder("settlement").getFolder("structure").get("_MOUNTAIN"))).name, icon, t.colors.minimap.mountain);
		this.sheet = sheet;
		SINGLES = SET*8;
		CAVE = new Ceiling(t, sheetC, icon);
	...
	void hoverInfo(GBox box, int tx, int ty) {
		...
		box.setResource(RESOURCES.STONE(), 1);
	}
	...
	public static class Ceiling extends Terrain.TerrainTile{
		...
		private Ceiling(Terrain t, TILE_SHEET s, SPRITE icon) {
			super(t, "cave", icon, t.colors.minimap.mountain.shade(1.5));
			this.sheet = s;
		...
		public class Opening extends Terrain.TerrainTile{
			...
			private Opening(Terrain t, TILE_SHEET sheet, SPRITE icon, int sheetStart) {
				super(t, "cave entrance", icon, t.colors.minimap.mountain.shade(1.5));
				this.sheet = sheet;
