...
public final class TBuilding extends INFO implements INDEXED {
	...
	public static final class TCollection extends RCollection<TBuilding> {

		private final LIST<TBuilding> all;
		public final TBuilding MUD;
		TCollection(Terrain terrain) throws IOException {
			super(KEY);
			...
			keys.add("_MUD");
			keys.add(data.getFiles());
			
			ArrayList<TBuilding> all = new ArrayList<>(keys.size());
	...
	TBuilding(String key, LISTE<TBuilding> all, Terrain t, Json data, Json text, PATH sg,
		...
		wall = new WallFull("BUILDING_" + key, t);
		broken = new WallBroken("BUILDING_" + key, t);
		roof = new Ceiling("BUILDING_" + key, t);
	...
	public class Wall extends BuildingComponent implements Diagonalizer{
		...
		private Wall(String key, Terrain t, boolean broken) {
			super(key, t, nameWall, iconWall, miniColor.shade(0.9), resource);
			this.broken = broken;
...
//			}
	...
	public class Ceiling extends BuildingComponent {
		...
		private Ceiling(String key, Terrain t) {
			super(key + "_CEILING", t, nameCeiling, iconCeiling, miniColor, resource);
		...
		public class Opening extends BuildingComponent implements Diagonalizer{
			...
			private Opening(String key, Terrain t) {
				super(key + "_OPENING", t, nameCeiling, iconCeiling, miniColor, resource);
