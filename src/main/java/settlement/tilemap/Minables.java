...
public class Minables {
	...
	public final MAP_OBJECTE<Minable> getter = new MAP_OBJECTE<Minable>() {
		...
		public void set(int tile, Minable object) {
			...
			if (object != old)
				TERRAIN().updateMiniMap(tile%TWIDTH, tile/TWIDTH);
		}
	...
	final GAMETILE CLEAR = new GAMETILE() {
		...
		@Override
		public MEDIUM getIcon() {
			return new ICON.MEDIUM.Twin(SPRITES.icons().m.pickaxe, SPRITES.icons().m.cancel);
		}
	...
	final GAMETILE INCREASE = new GAMETILE() {
		...
		@Override
		public MEDIUM getIcon() {
			return new ICON.MEDIUM.Twin(SPRITES.icons().m.pickaxe, SPRITES.icons().m.arrow_up);
		}
	...
	final GAMETILE DECREASE = new GAMETILE() {
		...
		@Override
		public MEDIUM getIcon() {
			return new ICON.MEDIUM.Twin(SPRITES.icons().m.pickaxe, SPRITES.icons().m.arrow_down);
		}
...
//		ran = ran >> 3;
		...
		if (iters > 0) {
			...
			int am = (int) ((a-iters)/3);
			int dd = ran%3;
			for (int i = 0; i < iters; i++) {
				int amm = CLAMP.i(am, 0, a);
				a -= amm;
				int tt = amm*8 + (ran&0x07);
				ran = ran >> 3;
				DIR dir = dirs[dd];
				dd++;
				dd %= 3;
