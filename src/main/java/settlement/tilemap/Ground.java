...
public class Ground extends TileMap.Resource{
	...
	private final byte[] data = new byte[C.SETTLE_TSIZE*C.SETTLE_TSIZE];
	...
	Ground(TileMap m) throws IOException{
...
//		this.cdry.set(SETT.ENV().climate().colorGroundDry);
	...
	public final class GROUND {
		...
		TextureCoords getTexture(int ran) {
			return sheet.getTexture(ran&16);
		}
	}
	...
	public final MAP_OBJECT<GROUND> getter = new MAP_OBJECT<Ground.GROUND>() {
		...
		public GROUND get(int tx, int ty) {
			if (SETT.IN_BOUNDS(tx, ty))
				return all.get((data[ty*C.SETTLE_TSIZE+tx] >> 4)&0x0F);
			return null;
		}
	...
	protected void clearAll() {
		for (int y = 0; y < C.SETTLE_TSIZE; y++){
			for (int x = 0; x < C.SETTLE_TSIZE; x++){
				data[y*C.SETTLE_TSIZE+x] = 0;
				minerals.CLEAR.placeFixed(x, y);
			}
		}
	} 
	...
	@Override
	protected COLOR miniColorPimped(ColorImp origional, int x, int y, boolean northern, boolean southern) {
		...
		for (DIR d : DIR.ALL) {
			if (TERRAIN().WATER.isWater(x+d.x(), y+d.y()) || TERRAIN().MOUNTAIN.is(x+d.x(), y+d.y())) {
