...
public final class TForest {
	...
	final TAmount amount = new TAmount(8, "Tree") {
	...
	public abstract class Tree extends Terrain.TerrainTile {
		...
		protected Tree(Terrain shared, int i) {
			super(shared, ¤¤name, SPRITES.icons().m.cancel, shared.colors.minimap.tree);
		}
	...
	public class Medium extends Tree {
		...
		@Override
		protected void placeRaw(int tx, int ty) {
	...
	private static final class Sprites {
		...
		private Sprites() throws IOException {
			...
			TILE_SHEET small = new ComposerThings.ITileSheet(p, 720, 296) {
			...
			TILE_SHEET medium = new ComposerThings.ITileSheet() {
			...
			this.small = make(SheetType.s1x1, small);
			this.small.d.shadowLength = 5;
			this.small.d.shadowHeight = 2;
			
			this.medium = make(SheetType.s2x2, medium);
