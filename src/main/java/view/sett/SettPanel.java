...
final class SettPanel{
	
	int i = 0;
	private GuiSection buttss = new GuiSection();
	
	SettPanel(UIPanelTop panel, SettView s){
...

		{
			...
			b = new GButt.BStat2(SPRITES.icons().s.human, new GStat() {
				...
				protected void clickA() {
					VIEW.s().ui.subjects.show();
				}
				
				@Override
				protected void renAction() {
					selectedSet(VIEW.s().ui.subjects.listActive());
				}
			}.hoverInfoSet(STATS.POP().POP.info().name);
			add(b);
			
			
			add(new StandingButt(HCLASS.CITIZEN, s.ui.standing));
			add(new StandingButt(HCLASS.SLAVE, s.ui.slaves));
			b = new Buttt(SPRITES.icons().s.noble,s.ui.nobles) {
			...
			b = new Buttt(SPRITES.icons().s.house, s.ui.home) {
			...
			b = new Buttt(SPRITES.icons().s.law, s.ui.law) {
			...
			b = new Buttt(SPRITES.icons().s.hammer, s.ui.rooms.main()) {
			...
			b = new Buttt(SPRITES.icons().s.sword, s.ui.army) {
