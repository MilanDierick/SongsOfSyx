...
final class Node extends GuiSection{
	...
	public static final COLOR Callocated = new ColorImp(0, 70, 70);
	...
	private class Content extends ClickableAbs{
		...
		protected void render(SPRITE_RENDERER r, float ds, boolean isActive, boolean isSelected, boolean isHovered) {
			...
			if (FACTIONS.player().tech.costOfNextWithRequired(tech) > FACTIONS.player().tech().available().get() || !tech.lockable.passes(FACTIONS.player())) {
				OPACITY.O50.bind();
...
//			return Cdormant;
		...
		public void hoverInfoGet(GUI_BOX text) {
			...
			
			{
				if (tech.levelMax == 1) {
					if (t.level(tech) == 1) {
						b.add(b.text().normalify2().add(DicMisc.¤¤Activated));
					}
				}else {
			...
			b.sep();
			{
				...
				tech.lockable.hover(text, FACTIONS.player());
				
				if (am > 0) {
					if (tech.lockable.all().size() == 0)
