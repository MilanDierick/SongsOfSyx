...
public class UIGoodsImport extends GuiSection{
	...
	static CharSequence ¤¤BestD = "¤Make a custom order of this goods from the trade partner with the most favourable price.";
	static CharSequence ¤¤Closest = "¤Closest";
	static CharSequence ¤¤ClosestD = "¤Make a speedy custom order of this goods from the trade partner that is closest.";
		...
		
		{
			...
			GButt.ButtPanel b = new GButt.ButtPanel(¤¤Best) {
				...
				protected void render(SPRITE_RENDERER r, float ds, boolean isActive, boolean isSelected,
						boolean isHovered) {
					isActive &= FACTIONS.pRel().pricesBuy.get(res.get()) > 0;
				...
				protected void clickA() {
					...
					if (f != null) {
						...
						VIEW.UI().factions.openBuy(f, res.get());
					}
				}
	...
	static RENDEROBJ miniControl(RESOURCE res, UIGoodsImport setting) {
		
		GuiSection s = new GuiSection();
		
		GETTER_IMP<RESOURCE> get = new GETTER.GETTER_IMP<RESOURCE>(res);
		
		s.add(UIGoodsImport.capBar(get, 64, 24));
		s.addRelBody(0, DIR.S, UIGoodsImport.slider(get, 64, 24));
			...
			GButt.ButtPanel b = new GButt.ButtPanel(UI.icons().s.cog) {
				...
				public void hoverInfoGet(GUI_BOX text) {
					...
					if (p != null)
						b.error(p);
					b.NL(4);
					p = SETT.ROOMS().IMPORT.tally.warning(res);
					if (p != null)
				...
				public void render(SPRITE_RENDERER r, float ds) {
					if (SETT.ROOMS().IMPORT.tally.capacity.get(res) == 0) {
						...
						if (SETT.ROOMS().IMPORT.tally.importWhenBelow.get(res) > 0) {
							...
							}else if (SETT.ROOMS().IMPORT.tally.warning(res) != null) {
	...
	private GuiSection priceH() {
		GuiSection s = new GuiSection();
		s.add(priceChart(FACTIONS.pRel().pricesBuy, DicRes.¤¤buyPrice, res, 8, 64));
	...
	static HOVERABLE capBar(GETTER<RESOURCE> res, int width, int height) {
		return new HOVERABLE.HoverableAbs(width, height) {
			...
			public void hoverInfoGet(GUI_BOX text) {
				...
				b.add(b.text().lablifySub().add(DicRes.¤¤Inbound));
				b.tab(6);
				...
				b.NL(8);
				b.add(b.text().lablifySub().add(DicRes.¤¤Importable));
				b.tab(6);
		...
		s.add(new HOVERABLE.HoverableAbs(450, 80) {
			...
			protected void render(SPRITE_RENDERER r, float ds, boolean isHovered) {
				...
				if (p != null) {
					...
					p = SETT.ROOMS().IMPORT.tally.warning(res.get());
					GCOLOR.UI().SOSO.hovered.bind();
					if (p != null)
						UI.FONT().S.render(r, p, body().x1()+8, body().y1()+8, body().width()-16, 1.0);
					COLOR.unbind();
