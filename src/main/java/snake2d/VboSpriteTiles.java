...
class VboSpriteTiles extends VboAbs{

	private final VboShaderAbs shader;

	static VboSpriteTiles getDeffered(SETTINGS sett, boolean opaque) {
		ShaderDebug shader = new ShaderDebug(sett.getNativeWidth(), sett.getNativeHeight(), false);
		return new VboSpriteTiles(shader, opaque);
	}

	public VboSpriteTiles(VboShaderAbs shader, boolean opaque) {
		super( GL_POINTS,
				1<<16,
				new VboAttribute(2, GL_SHORT, false, 2), // position upper left		4
				new VboAttribute(2, GL_UNSIGNED_BYTE, 1), // quad Dim	4
				new VboAttribute(2, GL_UNSIGNED_SHORT, 2), // texture coords1		4
				new VboAttribute(2, GL_UNSIGNED_SHORT, 2), // texture coords2		4
				new VboAttribute(2, GL_UNSIGNED_BYTE, 1), // texture coords width	4
	...
	private static class ShaderDebug extends VboShaderAbs {
		...
		ShaderDebug(float width, float height, boolean debug) {
			...
			String VERTEX = "#version 330 core" + "\n"
				...
				+ "layout(location = 1) in uvec2 in_dim;" + "\n" 
				...
				+ "void main(){" + "\n" 
					...
					+ "gPos2 = vec2((vec2(in_position1.x+in_dim.x, in_position1.y+in_dim.y) * screen)+trans);" + "\n" 
	...
	void render(TextureCoords t, TextureCoords to, int x1, int x2, int y1, int y2, COLOR color, OPACITY opacity) {
		...
		buffer.put((byte) (x2-x1)).put((byte) (y2-y1));
		buffer.putShort(t.x1()).putShort(t.y1());
		buffer.putShort(to.x1()).putShort(to.y1());
		buffer.put((byte) (t.x2()-t.x1())).put((byte) (t.y2()-t.y1()));
