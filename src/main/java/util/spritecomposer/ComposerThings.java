...
public final class ComposerThings {
	...
	public static abstract class IIcon{
		
		public static class SMALL extends ICON.SMALL implements TextureCoords{

			private final short x1,y1;
			final static int pixelSize = 16;
			...
			public static ICON.SMALL get(SpriteData d) throws IOException {
				int x1 = d.x1;
				int y1 = d.y1;
				int size = d.width;
				if (size != pixelSize || size != d.height)
					throw new IOException("icon is wrong size! " + + d.width + " " +d.height);
				return new IIcon.SMALL(x1, y1);
			}
			
			public static LIST<ICON.SMALL> get(SpriteData[] data) throws IOException{
				ArrayList<ICON.SMALL> res = new ArrayList<>(data.length);
			...
			@Override
			public void render(SPRITE_RENDERER r, int X1, int X2, int Y1, int Y2) {
				r.renderSprite(X1, X2, Y1, Y2, this);
		...
		public static class MEDIUM extends ICON.MEDIUM implements TextureCoords{

			private final short x1,y1;
			final static int pixelSize = 24;
			
			public MEDIUM(int x1, int y1){
				this.x1 = (short) x1;
				this.y1 = (short) y1;
			}

			public static ICON.MEDIUM get(SpriteData d) throws IOException {
				int x1 = d.x1;
				int y1 = d.y1;
				int size = d.width;
				if (size != pixelSize || size != d.height)
					throw new IOException("icon is wrong size! " + + d.width + " " +d.height);
				return new IIcon.MEDIUM(x1, y1);
			}
			
			public static LIST<ICON.MEDIUM> get(SpriteData[] data) throws IOException{
				ArrayList<ICON.MEDIUM> res = new ArrayList<>(data.length);
				for (int i = 0; i < data.length; i++)
					res.add(get(data[i]));
				return res;
			}
		...
		public static class LARGE extends ICON.BIG implements TextureCoords{

			private final short x1,y1;
			final static int pixelSize = 32;
			
			public LARGE(int x1, int y1){
				this.x1 = (short) x1;
				this.y1 = (short) y1;
			}

			public static ICON.BIG get(SpriteData d) throws IOException {
				int x1 = d.x1;
				int y1 = d.y1;
				int size = d.width;
				if (size != pixelSize || size != d.height)
					throw new IOException("icon is wrong size! " + + d.width + " " +d.height);
				return new IIcon.LARGE(x1, y1);
			}
			
			public static LIST<ICON.BIG> get(SpriteData[] data) throws IOException{
				ArrayList<ICON.BIG> res = new ArrayList<>(data.length);
				for (int i = 0; i < data.length; i++)
					res.add(get(data[i]));
				return res;
			}
