...
public abstract class GResSelector extends GuiSection{
	...
	public GResSelector(boolean includenull){
		this(RESOURCES.ALL(), includenull);
	}
	
	public GResSelector(LIST<RESOURCE> list, boolean includenull){
		int i = 0;
		if (includenull) {
	...
	private class Resbutt extends GButt.ButtPanel{
		...
		public void hoverInfoGet(GUI_BOX text) {
			if (res == null)
				...
				GResSelector.this.hoverResource(res, (GBox)text);
		}
		...
		protected void renAction() {
			selectedSet(GResSelector.this.getResource() == res);
		}
