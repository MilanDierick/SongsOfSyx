...
public class Icons{
	...
	private Icon get(String relPath, Json j, String key) throws IOException{
		...
		if (!makers.containsKey(ss[0])) {
			GAME.Warn(j.errorGet(PATHS.SPRITE().getFolder("icon") + File.separator + ss[0] + " does not exists", relPath));
			return m.DUMMY;
		}
		...
		for (int i = 1; i < ss.length-2; i++) {
			if (!p.existsFolder(ss[i])) {
				GAME.Warn(j.errorGet(""+p.get() + File.separator + ss[i] + " does not exists", relPath));
				return m.DUMMY;
			}
			p = p.getFolder(ss[i]);
		}
		...
		String rel = relPath.substring(ss[0].length(), relPath.length());

		return m.get(j, key, rel);
	}
	...
	public static class M extends IconMaker{
		...
		public final Icon clear_tunnel = get();
		public final Icon job_sleep = get();
		public final Icon job_awake = get();
		public final Icon agriculture = get();
		public final Icon axe = get();
		...
		public final Icon clearstone = get();
		public final Icon tunnel_fill = get();
		public final Icon religion = get();
		public final Icon clear_all = get();
		...
		public final Icon polishCave = get();
		...
		public final Icon shovel = get();
		public final Icon urn = get();
		public final Icon fillWater = get();
		public final Icon digCanal = get();
		...
		public final Icon sanitation = get();
		...
		public final Icon knowledge = get();
	...
	public static class S extends IconMaker{
		...
		public final IconS hammer_lying = get();
		public final IconS sprout = get();
		public final IconS fence = get();
		...
		private IconS get() throws IOException {
			int k = i;
			i++;
			return new IconS(sheet("_Icons", null), k);
		}

		private final static COLOR mask = new ColorImp(142, 134, 107);
		
		public static class IconS extends IconSheet {

			public final int index;
			
			IconS(TILE_SHEET sheet, int tile) {
				super(Icon.S, sheet, tile);
				index = all.add(this);
			}
