	...
	private static class BMil {
		...
		BMil(){
			...
			for (RDBuilding b : RD.BUILDINGS().all) {
				double v = Math.max(b.boosters().max(RD.MILITARY().bgarrison), b.boosters().max(RD.MILITARY().defences));
				if (!b.AIBuild || v <= 0)
					continue;
	...
	private static class BReligion {
		...
		BReligion(){
			...
			for (RDReligion rr : RD.RELIGION().all()) {
				boosts.put(rr.religion.boostable.key, rr.religion);
			}
		...
		void build(Region reg, RealmBuilder builder) {
			...
			for (RDRace r : RD.RACES().all) {
				...
				for (RDRaceEdict ee : r.edicts) {
					ee.toggled.set(reg, 0);
				}
				
				if (v < 0) {
					v = -v;
					int i = (int) Math.round(v*r.edicts.size())-1;
					i = CLAMP.i(i, 0, r.edicts.size()-1);
					if (i >= 0) {
						r.edicts.get(i).toggled.set(reg, 1);
					}else {
